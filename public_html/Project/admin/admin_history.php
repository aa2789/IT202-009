...
if(isset($_POST["filter"])){
    if(isset($_POST["price"])&&!empty($_POST["price"])){
        ...
        $query.=" ORDER BY created ASC";
         ...
         $query.=" WHERE created BETWEEN '$start' AND '$end' ";
    }
}
...
<?php foreach($orders as $index=>$order): ?>
   ...
   <?php if($index==0): ?>
    ...
    <tr>
        ...
        <?php $totalPrice+=(number_format($price/100,2)) ?>
...
<h4>Combined Price of Search Results: <?php echo "$",number_format($totalPrice,2) ?> </h4>
<h5> Select Page </h5>
<?php
 for($page=1;$page<=$number_of_pages;$page++){
    
    $loc="admin_history.php?page=$page";
